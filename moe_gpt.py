...
class FeedForward(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        # Expects hidden_dim instead of using 4*embed_dim
...
class MoEFeedForward(nn.Module):
    def __init__(self, cfg):
        ...
        # Fully connected experts
        self.fc1 = nn.ModuleList([nn.Linear(self.embed_dim, self.hidden_dim) for _ in range(self.num_experts)])
        self.fc2 = nn.ModuleList([nn.Linear(self.embed_dim, self.hidden_dim) for _ in range(self.num_experts)])
        self.fc3 = nn.ModuleList([nn.Linear(self.hidden_dim, self.embed_dim) for _ in range(self.num_experts)])
    
    def forward(self, x):
        # x: (batch, num_tokens, embed_dim)
        batch, num_tokens, _ = x.shape
        
        # Router and expert probabilities
        scores = self.gate(x) # (batch, num_tokens, num_experts)
        topk_scores, topk_indices = torch.topk( # (batch, num_tokens, num_experts_per_token)
            scores, 
            k=self.num_experts_per_token, 
            dim=-1
        )
        topk_probs = F.softmax(topk_scores, dim=-1) # (batch, num_tokens, num_experts_per_token)

        # Flatten input/output tensors
        x_flat = x.reshape(batch * num_tokens, -1) # (batch * num_tokens, embed_dim)
        out_flat = torch.zeros(batch * num_tokens, self.embed_dim, device=x.device) # (batch * num_tokens, embed_dim)
        ...
        for expert_id_tensor in unique_experts: # Iterate over unique experts
            expert_id = expert_id_tensor.item()
            mask = topk_indices_flat == expert_id # (batch * num_tokens, num_experts_per_token)
            
            if not mask.any():
                # Skip if no tokens assigned to this expert (just in case)
                continue

            token_mask = mask.any(dim=-1) # Collapse across experts --> (batch * num_tokens,)
            selected_idx = token_mask.nonzero(as_tuple=False).squeeze(-1) # Indices where token_mask is True
            if selected_idx.numel() == 0:
                # Skip if no tokens assigned to this expert (just in case)
                continue
            
            # Forward pass through expert
            expert_input = x_flat.index_select(dim=0, index=selected_idx) # (num_selected_tokens, embed_dim)
            # Silu activation: silu(fc1(x)) * fc2(x) --> (num_selected_tokens, hidden_dim)
            hidden = F.silu(self.fc1[expert_id](expert_input)) * self.fc2[expert_id](expert_input)
            expert_out = self.fc3[expert_id](hidden) # (num_selected_tokens, embed_dim)

            # Expert weight (probability of this expert for each selected token)
            ...
            # Add expert output to output tensor (weighted by expert probability)
            out_flat.index_add_(0, selected_idx, expert_out * selected_probs.unsqueeze(-1))
        
        return out_flat.reshape(batch, num_tokens, self.embed_dim) # (batch, num_tokens, embed_dim)
