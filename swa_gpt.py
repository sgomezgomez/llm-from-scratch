...
class GPTModel(nn.Module):
    def __init__(self, cfg: dict):
        ...
        self.sliding_window_size = self.cfg["sliding_window_size"]
        self.sliding_window_stride = self.cfg["sliding_window_stride"]
        # Switched to KV cached multi head attention
        #self.transformer_blocks = nn.Sequential(*[TransformerBlock(self.cfg) for _ in range(self.cfg["num_layers"])])
        # Create transformer blocks with SWA and MHA layers using K:1 pattern (k = sliding_window_stride)
        # K:1 pattern means K SWA layers are followed by 1 MHA layer
        blocks = []
        K = int(self.sliding_window_stride)
        for i in range(self.cfg["num_layers"]):
            block = TransformerBlock(self.cfg)
            # Pattern: K SWA layers (0 to K-1), then 1 MHA (K), then K SWA (K+1 to 2K), then 1 MHA (2K+1), etc.
            # Use (i+1) % (K+1) == 0 to place MHA at positions K, 2K+1, 3K+2, ...
            if (i + 1) % (K + 1) == 0:
                block.attention.sliding_window_size = None  # No SWA (MHA layer)
            blocks.append(block)
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.attention = MultiHeadSlidingWindowAttention(
            d_in=cfg["embed_dim"],
            d_out=cfg["embed_dim"],
...
# Variation of the multi head attention using KV caching and sliding window attention (SWA)
# Adapted from: https://github.com/rasbt/LLMs-from-scratch/blob/main/ch04/06_swa/gpt_with_kv_swa.py
class MultiHeadSlidingWindowAttention(nn.Module):
    def __init__(self, 
                 d_in: int, 
                 d_out: int, 
                 context_len: int, 
        ...
        self.sliding_window_size = sliding_window_size or self.window_size # Sliding window size for SWA
    ...
    def forward(self, x: torch.Tensor, kv_cache: bool = False) -> torch.Tensor:
        ...
        # Sliding window attention -- including causal mask
        swa_att_mask = None
        if self.sliding_window_size is not None:
            if Tk > self.sliding_window_size:
                k = k[:, :, -self.sliding_window_size:, :]
                v = v[:, :, -self.sliding_window_size:, :]
                Tk = self.sliding_window_size
            # Skip mask creation when Tq == 1 (single token inference)
            # Keys are already cropped to window_size, and all keys are in the past (no causal constraint needed)
            if Tq > 1:
                # Lower triangular mask (window constraint) - can't attend tokens before window size
                # For each query position i, can only attend to keys at positions >= max(0, i - window_size + 1)
                # Create indices: query positions (rows) and key positions (cols)
                q_positions = torch.arange(Tq, device=q.device).unsqueeze(1)  # (Tq, 1) -- column vector
                k_positions = torch.arange(Tk, device=q.device).unsqueeze(0)  # (1, Tk) -- row vector
                # Window constraint: k_positions >= max(0, q_positions - window_size + 1)
                window_start = torch.clamp(q_positions - self.sliding_window_size + 1, min=0) # (Tq, 1)
                window_mask = k_positions >= window_start  # (Tq, Tk)
                # Upper triangular mask (causal) - can't attend to future tokens
                causal_mask = torch.tril(torch.ones((Tq, Tk), dtype=torch.bool, device=q.device))
                # Combine both masks: True = keep, False = mask
                swa_att_mask = causal_mask & window_mask
        ...
        if kv_cache is None or not kv_cache or (Tk == Tq):
            # Dot product with SWA and Causal mask
            if swa_att_mask is not None:
                context_vec = F.scaled_dot_product_attention(
                    q, k, v,
                    attn_mask=swa_att_mask,
        ...
        # Only one token in current batch -- No SWA or Causal mask or dropout
