...
class Llama3Model(nn.Module):
    ...
    def precompute_rope(self):
        theta_base = self.cfg["rope_base"] if "rope_base" in self.cfg else 500_000
        freq_config = self.cfg["rope_freq"] if "rope_freq" in self.cfg else None
        ...
        if freq_config is not None:
            ...
            inv_freq_llama = torch.where(
                ...
                inv_freq / freq_config["low_freq_factor"],
                inv_freq
            )
            ...
            inv_freq_llama = torch.where(is_medium_freq, smoot_inv_freq, inv_freq_llama)
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.attention = GroupedQueryAttention_KVCache(
            d_in=cfg["embed_dim"],
            d_out=cfg["embed_dim"],
...
# Grouped Query Attention (GQA) using KV caching
# KV caching is useful to speed up inference
...
# Also adapted from: https://github.com/rasbt/LLMs-from-scratch/blob/main/ch05/07_gpt_to_llama/converting-llama2-to-llama3.ipynb
class GroupedQueryAttention_KVCache(nn.Module):
    def __init__(self, 
                 d_in: int, # Embedding dimension
                 d_out: int, # Hidden dimension
                 context_len: int,
                 # No dropout for llama 3
                 num_heads: int,
                 num_kv_groups: int,
                 dtype: torch.dtype, 
        ...
        self.num_kv_groups = num_kv_groups # Number of groups of key-value pairs
        self.head_dim = d_out // num_heads # Reduce projection dim to match desired output dim
        self.group_size = self.num_heads // self.num_kv_groups # Size of each key-value group
        self.context_len = context_len
        self.dtype = dtype # Store dtype for cache initialization
        # Single query projection layer
        self.q = nn.Linear(d_in, d_out, bias=False, dtype=dtype)
        # Combine keys and values into a single grouped linear layer (grouped key-values)
        self.kv = nn.Linear(d_in, 2 * self.num_kv_groups * self.head_dim, bias=False, dtype=dtype)
    ...
    def forward(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, kv_cache: bool = False) -> torch.Tensor:
        ...
        q = self.q(x) # queries --> (batch, num_tokens, d_out)
        kv = self.kv(x) # key-values --> (batch, num_tokens, 2 * self.num_kv_groups * self.head_dim)

        # With KV caching, we only project the next token id after the first prediction (not the entire sequence)
        # This means num_tokens == Tq == 1 in those cases
        q = q.view(batch, num_tokens, self.num_heads, self.head_dim).permute(0, 2, 1, 3) # (batch, num_heads, num_tokens, head_dim)
        kv = kv.view(batch, num_tokens, 2, self.num_kv_groups, self.head_dim).permute(2, 0, 3, 1, 4) # (2, batch, num_kv_groups, num_tokens, head_dim)
        k_new, v_new = kv # 2 times (batch, num_kv_groups, num_tokens, head_dim)
        # Apply RoPE -- QK rotary embedding
        q = self.apply_rope(q, cos, sin) # (batch, num_heads, num_tokens, head_dim)
        k_new = self.apply_rope(k_new, cos, sin) # (batch, num_kv_groups, num_tokens, head_dim)

        # Expand keys-values groups to match number of heads
        # Before repeat_interleave: [K1, K2, ..., K_num_kv_groups]
        # After repeat_interleave: [K1, K1, ..., K1, K2, K2, ..., K2, ..., K_num_kv_groups, K_num_kv_groups, ..., K_num_kv_groups]
        k_new = k_new.repeat_interleave(self.group_size, dim=1) # (batch, num_heads, num_tokens, head_dim)
        v_new = v_new.repeat_interleave(self.group_size, dim=1) # (batch, num_heads, num_tokens, head_dim)
...
# Simple wrapper arund tiktoken for llama 3 special ids
class Llama3Tokenizer:
    def __init__(self, model_name: str = "Llama-3-8B"):
        access_token = access_config.huggingface_api_key
        login(access_token)
        tokenizer_file = hf_hub_download(
            repo_id="meta-llama/Meta-" + model_name,
            filename="original/tokenizer.model",
            local_dir=model_name
        )

        mergeable = load_tiktoken_bpe(tokenizer_file)
